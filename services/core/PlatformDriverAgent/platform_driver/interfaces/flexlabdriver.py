...
class C1ARegister(BaseRegister):
    """
    ...
    def __init__(self, csv_path, read_only, pointName, units, reg_type,
        ...
        super(C1ARegister, self).__init__("byte", read_only, pointName, units,
        ...
        with open(self.csv_path, "r") as csv_device:
            ...
            for point in reader:
                if point["Point Name"] == self.point_name:
                    ...
                    points.append(point)
        print(os.path.abspath(self.csv_path))
        if not found:
...
class Interface(BasicRevert, BaseInterface):
    ...
    def configure(self, config_dict, registry_config_str):
        ...
        print(sys.path)
        if True:  # not os.path.isfile(self.csv_path):
            _log.info("Creating csv 'device'")
            CSV_DEFAULT = []
            with open(self.csv_path, "w+") as csv_device:
                writer = DictWriter(csv_device, fieldnames=CSV_FIELDNAMES)
                writer.writeheader()
                for index, regDef in enumerate(registry_config_str):
                    point = {"Point Name": regDef.get('Volttron Point Name'),
                             "Point Value": 0}
                    CSV_DEFAULT.append(point)
